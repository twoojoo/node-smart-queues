...
> **Why sorted sets?** that's because Redis' lists (which would a have been a better choiche, from a performace point of view) lacks a proper TTL system. To provide this feature (and to mantain the storage recovery compatibility when dynamically adding or removing the TTL), I prefered to take advantage of Redis' sorted sets, using the enqueue timestamp as score, which is automatically checked for cleanups when a TTL is provided. This mean that the enqueing and dequeing of an item time complexity will be **O(log n)** instead of **O(1)**, which shouldn't be a big performance loss in a queue context, unless items are supposed to stay enqueued for a long time and a lot of items get enqueued.
